...
} from "@registry/new-york/components/timeframe-tabs/timeframe-tabs";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
    ...
    >
      ...
      <DropdownMenu>
        <DropdownMenuTrigger
          ...
          className="group text-secondary-foreground/60 hover:text-secondary-foreground/90 hover:bg-muted/60 focus-visible:ring-ring/50 data-[state=open]:text-secondary-foreground data-[state=open]:bg-muted relative ml-0.5 flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-md text-sm font-medium transition-colors duration-150 outline-none focus-visible:ring-2"
        >
        ...
        <DropdownMenuContent align="end" className="mt-2 w-40">
          {timeframeGroups.map((group, i) => (
            <React.Fragment key={group.label}>
              <DropdownMenuLabel className="text-muted-foreground/70 text-xs font-semibold tracking-wider uppercase">
                {group.label}
              </DropdownMenuLabel>
